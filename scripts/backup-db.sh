#!/bin/bash
# Nidus — Daily database backup with retention
set -e

BACKUP_DIR="${BACKUP_DIR:-/root/nidus/backups/db}"
...
# Cleanup old backups
find "$BACKUP_DIR" -name "nidus_*.sql.gz" -mtime +$RETENTION_DAYS -delete 2>/dev/null || true

# Keep only last 30 backups if retention is set
BACKUP_COUNT=$(ls -1 "$BACKUP_DIR"/nidus_*.sql.gz 2>/dev/null | wc -l)
if [ "$BACKUP_COUNT" -gt 30 ]; then
    ls -1t "$BACKUP_DIR"/nidus_*.sql.gz | tail -n +31 | xargs rm -f
