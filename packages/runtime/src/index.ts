...
import type { ProjectConfig, DeploymentResult, Framework } from "@stackrun/shared"
...
export async function buildProject(config: ProjectConfig, sourceDir: string): Promise<DeploymentResult> {
  try {
    ...
    const dockerfilePath = join(sourceDir, "Dockerfile.stackrun")
    writeFileSync(dockerfilePath, dockerfile)

    const tag = `stackrun/${config.slug}:latest`
...
export async function deployProject(config: ProjectConfig, port: number): Promise<DeploymentResult> {
  try {
    const tag = `stackrun/${config.slug}:latest`
    const containerName = `stackrun-${config.slug}`
    ...
    const container = spawn("docker", [
      "run", "-d",
      "--name", containerName,
      "--label", `stackrun.project=${config.slug}`,
...
export async function stopProject(slug: string): Promise<void> {
  execSync(`docker rm -f stackrun-${slug} 2>/dev/null || true`, { stdio: "pipe" })
}
